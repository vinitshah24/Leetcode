...
Each row of this table indicates that the stock which has stock_name had an operation on day operation_day with price.
...
select
    stock_name,
    (sold_total - buy_total) as capital_gain_loss
from (
    (
        select stock_name, sum(price) as sold_total
        from stocks
