...
INSERT INTO request_accepted VALUES (1, 2), (1, 3), (2, 3), (3, 4);
SELECT * FROM request_accepted;


-- Solution
select requester_id, sum(user_count) as num
from (
    (   -- REQUESTER COUNT
        select requester_id, count(distinct accepter_id) as user_count
    ...
    (
        -- ACCEPTER COUNT
        select accepter_id, count(distinct requester_id) as user_count
        from request_accepted
