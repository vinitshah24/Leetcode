...
-- In this case, you just need to simply count the total accepted requests (no matter whether they are in original requests),
...
Select
ifnull((
        select distinct round((select count(*) from t2) / ( select count(*) from t1),2)
		from t1,t2
    ),0) 'accept_rate'
